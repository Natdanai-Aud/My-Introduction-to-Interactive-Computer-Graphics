...
if obj is None or obj.type != 'MESH':
    ...
    if color_attributes is None or len(color_attributes) == 0:
        raise RuntimeError("ไม่พบ Color Attribute ในโมเดล — โปรดสร้าง Vertex Color ก่อน")
    ...
    if color_domain == 'POINT':
        ...
        bm.free()
        raise RuntimeError(
            f"Color Attribute '{color_name}' ใช้ domain {color_domain} "
            "ซึ่ง exporter นี้รองรับเฉพาะ POINT และ CORNER"
        )

    if color_data_type == 'FLOAT_COLOR':
        color_layer = color_layers.float_color.get(color_name)
    elif color_data_type == 'BYTE_COLOR':
    ...
    if color_layer is None:
        bm.free()
        raise RuntimeError(
            f"ไม่สามารถอ่าน Color Attribute '{color_name}' "
            f"ชนิด {color_data_type} ได้"
        )
